...
RSpec.describe LiquidFilesController, type: :controller do
  ...
  let(:elif_result) { "<p>{% elsif 'agreeableness' | UserAttribute < 0 %}, Congrats on your First Win!" }
  ...
  let(:blank_if_result) { "<p>{%if neuroticism == nil %}, Congrats on your First Win!" }
  ...
  describe 'replace_conditionals' do
    ...
    it { expect(subject.send(:replace_conditionals, blank_if_string)).to eq blank_if_string }
